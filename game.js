...
function openEventTab() {
    ...
    if (isNYEvent) {
        ...
        html += `<div style="text-align:center; color:#aaa; font-size:12px; padding:20px;">현재 참여 가능한 이벤트가 없습니다.</div>`;
    }
...
function doAcademy() {
    ...
    updateAllStats();
    showMessage("훈련을 무사히 마쳤습니다!<br>파티장의 주요 스탯(힘,민첩,지능,행운)이 +1씩 영구 상승했습니다.", openGuild);
}
...
function returnToTown(isGameOver) {
    ...
    if (isGameOver) {
        ...
        player.gold = Math.floor(player.gold / 2); player.items = { food: 0, hpPotion: 0, mpPotion: 0, ore: 0, envelope: player.items.envelope || 0 }; player.hunger = 0; player.fatigue = 0;
