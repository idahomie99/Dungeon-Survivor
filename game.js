...
const jobData = {
    '바바리안': { img: 'image/barbarian.png', desc: '강력한 체력과 공격력으로 적을 분쇄하는 전사', atk:25, matk:10, str:20, dex:5, luk:10, int:10, maxHp:180, maxMp:50, def:15, skills: [{n:'분쇄', lv:1, mp:10, mult:1.5}, {n:'대지 강타', lv:3, mp:20, mult:2.2}, {n:'광전사의 분노', lv:5, mp:35, mult:3.0}] },
    '엘프': { img: 'image/elf.png', desc: '빠른 몸놀림과 날카로운 활시위로 적을 제압하는 궁수', atk:20, matk:10, str:15, dex:25, luk:10, int:10, maxHp:110, maxMp:60, def:5, skills: [{n:'연사', lv:1, mp:10, mult:1.5}, {n:'관통 화살', lv:3, mp:20, mult:2.2}, {n:'폭풍의 시위', lv:5, mp:35, mult:3.0}] },
    '드워프': { img: 'image/dwarf.png', desc: '강철 같은 방어력으로 최전방을 지키는 철벽의 수호자', atk:22, matk:10, str:15, dex:10, luk:10, int:10, maxHp:220, maxMp:50, def:20, skills: [{n:'방패 치기', lv:1, mp:10, mult:1.5}, {n:'지진', lv:3, mp:20, mult:2.2}, {n:'철벽의 일격', lv:5, mp:35, mult:3.0}] },
    '마법사': { img: 'image/magician.png', desc: '강력한 마력으로 적을 불태우는 지식인', atk:10, matk:25, str:10, dex:10, luk:15, int:20, maxHp:80, maxMp:150, def:5, skills: [{n:'매직 애로우', lv:1, mp:15, mult:1.6}, {n:'파이어볼', lv:3, mp:30, mult:2.4}, {n:'메테오', lv:5, mp:50, mult:3.5}] },
    '도적': { img: 'image/assassin.png', desc: '행운을 이용해 치명적인 한 방을 노리는 암살자', atk:22, matk:10, str:10, dex:20, luk:25, int:10, maxHp:90, maxMp:50, def:5, skills: [{n:'기습', lv:1, mp:10, mult:1.5}, {n:'그림자 베기', lv:3, mp:20, mult:2.2}, {n:'암살', lv:5, mp:35, mult:3.0}] },
    '소환사': { img: 'image/summoner.png', desc: '소환수의 힘을 빌려 전투를 지휘하는 지휘관', atk:10, matk:20, str:10, dex:15, luk:10, int:25, maxHp:100, maxMp:120, def:5, skills: [{n:'마력탄', lv:1, mp:15, mult:1.5}, {n:'정령의 분노', lv:3, mp:30, mult:2.3}, {n:'고대 마수 소환', lv:5, mp:50, mult:3.3}] }
};

let player = {
    gold: 1000, day: 1, rank: 1, level: 1, exp: 0, maxExp: 100, hunger: 100, maxHunger: 100, fatigue: 0,
    inDungeon: false, dungeonDay: 1, maxDungeonDay: 7, floor: 1, turn: 0, maxTurn: 20,
    ...
    db.collection("saves").doc(uid).get().then((doc) => {
        ...
        if (doc.exists) {
            ...
            btns.innerHTML = `
                ...
                <p style="color:#e53935; font-size:11.5px; text-align:center; margin-top:-5px; letter-spacing:0px;">※ 새로 시작하면 기존 데이터는 영구 삭제됩니다.</p>
            `;
        } else {
            text.innerHTML = "저장된 데이터가 없습니다. 새로운 모험을 시작합니다.";
            btns.innerHTML = `
                <button class="btn" style="border-color:#4caf50; color:#4caf50; font-weight:bold; padding:16px;" onclick="startNewGame()">새로하기</button>
            `;
        }
    }).catch((error) => {
...
function startNewGameConfirm() {
    showMessage("정말로 기존 데이터를 삭제하고 새로 시작하시겠습니까?", null, [
        {txt: "네 (삭제)", act: "closeModal(); startNewGame()"}, {txt: "아니오", act: "closeModal()"}
    ]);
}
...
function fadeTransition(callback) {
    ...
    setTimeout(() => { callback(); overlay.style.opacity = '0'; }, 300);
}
...
function showMessage(msg, callback, buttons) {
    ...
    if (buttons && btnArea) {
        btnArea.innerHTML = buttons.map(b => `<button class="modal-btn" onclick="${b.act}">${b.txt}</button>`).join('');
    } else if (btnArea) {
...
function updateAllStats() {
    ...
    if(hudArea) hudArea.innerHTML = hudHtml;
    
    const expBar = document.getElementById('exp-bar');
    if(expBar) expBar.style.width = `${(player.exp / player.maxExp) * 100}%`;
    const expText = document.getElementById('exp-text');
    if(expText) expText.innerText = `Lv.${player.level} (${Math.floor(player.exp)}/${player.maxExp})`;

    saveGame(); // 상태 갱신될 때마다 서버 저장
}
...
function levelUp() {
    ...
    player.party.forEach(p => {
        ...
        if(p.isPlayer) {
            let ts = getTotalStats();
            p.hp = ts.maxHp; p.mp = ts.maxMp;
        } else {
...
function passTime(days) {
    ...
    for(let i=0; i<days; i++) {
        player.day++;
        player.orePrice = Math.floor(500 * (0.8 + Math.random() * 0.4)); 
    }
    let newMonth = Math.floor((player.day - 1) / 30) + 1;
    if (oldMonth !== newMonth) {
        player.dungeonEnteredThisMonth = false;
        player.recruitUsedThisMonth = false;
        guildRecruit = null;
        ...
        if (player.gold < tax) {
            ...
            if(currentUser) db.collection("saves").doc(currentUser.uid).delete(); 
            fadeTransition(() => {
                showMessage(`[처형] 왕가의 세금 ${tax}G를 납부하지 못했습니다.<br>당신은 반역죄로 즉결 처형당했습니다...`, () => {
                    location.reload(); 
                });
            });
            return true;
        ...
        player.gold -= tax;
        showMessage(`새로운 달(${newMonth}월)이 밝았습니다!<br><br>왕가의 보호비 명목으로 ${tax}G (1000G + 재산의 20%)가 세금으로 징수되었습니다.<br>던전 포탈과 길드 모집이 갱신되었습니다.`, renderTownUI);
        return true;
    }
    return false;
}
...
function getTotalStats() {
    ...
    for (let slot in player.equipped) {
        if (slot === "반지") {
            player.equipped["반지"].forEach(ring => { for (let st in ring.stats) s[st] += ring.stats[st]; });
        } else if (slot !== "정수" && player.equipped[slot]) {
            let eq = player.equipped[slot];
            for (let st in eq.stats) s[st] += eq.stats[st];
        }
    }
    let essenceBonus = 0;
    player.equipped["정수"].forEach(ess => { essenceBonus += (ess.tier * 0.01); });
...
function promptNickname(job) {
    showMessage(`[캐릭터 생성]<br>선택한 직업: <b>${job}</b><br><br>사용할 닉네임을 입력해주세요:<br><input type="text" id="nickname-input" value="모험가" style="width:100%; padding:8px; margin-top:8px; background:#222; border:1px solid #444; color:#fff; border-radius:4px;">`, null, [
        {txt: "결정", act: "confirmNickname('" + job + "')"}
    ]);
}

function confirmNickname(job) {
    let input = document.getElementById('nickname-input');
    let nickname = input ? input.value.trim() : "모험가";
    if(!nickname) nickname = "모험가";
    closeModal();
    selectClass(job, nickname);
}
...
function showTutorial() {
    let msg = `
        <div style="font-weight:bold; color:#ffd54f; font-size:15px; margin-bottom:12px; text-align:center;">[ 던전 서바이버 가이드 ]</div>
        어둠의 세력이 지배하는 이 세계에서 살아남기 위해 오셨군요.<br><br>
        <b style="color:#e53935">1. 왕가의 세금 (주의!)</b><br>매월 1일, 보호비 명목으로 <span style="color:#ffc107">1000G + 전 재산의 20%</span>를 납부해야 합니다. <b>납부하지 못하면 즉시 처형당해 게임이 끝납니다!</b><br><br>
        <b style="color:#00e5ff">2. 마을 시설 & 파티 규칙</b><br>
        - <b>길드</b>: 동료 섭외/방출. <b>(던전 내 획득 골드와 원석은 파티원 수대로 1/N 균등 분배되며, 장비와 정수는 파티장이 온전히 독식합니다.)</b><br>
        - <b>일터</b>: 돈을 벌며 숙련도를 올립니다.<br>
        - <b>병원</b>: 골드를 지불하고 기절한 동료를 치료합니다.<br>
        - <b>상점</b>: 포션 구매 및 전리품 판매 (장착 중인 아이템 판매 불가)<br><br>
        <b style="color:#ce93d8">3. 던전 (포탈)</b><br>
        던전은 매월 1회 입장하며 <b>기본 7일간</b> 머물 수 있습니다. 각 층에서 <b>비석</b>을 찾아 다음 층으로 넘어가면 <b>유지 기간이 5일 추가</b>됩니다.
    `;
    ...
    if(actionArea) {
        actionArea.innerHTML = `
            <div class="action-grid">
                ...
                <div class="icon-btn" onclick="fadeTransition(() => openInventory('town'))">${icons.bag}<span>가방</span></div>
            </div>
        `;
    }
}
...
function openHospital() {
    ...
    for(let i=1; i<player.party.length; i++) {
        ...
        if (p.hp <= 0) {
            ...
            html += `<button class="btn" style="border-color:#e53935;" onclick="healCompanion(${i}, ${cost})">${p.name} 치료 (${cost}G)</button>`;
        }
    }
...
function healCompanion(index, cost) {
    ...
    player.gold -= cost;
    player.party[index].hp = player.party[index].maxHp;
    updateAllStats();
    showMessage(`${player.party[index].name}의 치료가 완료되었습니다.`, openHospital);
}

function openGuild() {
    const sceneText = document.getElementById('scene-text');
    const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `[용병 길드]<br>새로운 동료를 찾거나 승급할 수 있습니다.<br>최대 6인 구성, 중복 직업 불가.`;
    actionArea.innerHTML = `
        <div class="action-grid" style="grid-template-columns: 1fr 1fr;">
            ...
            <div class="icon-btn" onclick="showMessage('승급 퀘스트는 준비 중입니다.')">${icons.attack}<span>승급 퀘스트</span></div>
...
function showRecruit() {
    ...
    if (!guildRecruit) {
        if (Math.random() < 0.3) {
            player.recruitUsedThisMonth = true;
            return showMessage("오늘은 길드에 쓸만한 인재가 보이지 않습니다.");
        }
        ...
        let jName = availableJobs[Math.floor(Math.random() * availableJobs.length)];
        let d = jobData[jName];
        let mRank = Math.floor(Math.random() * player.rank) + 1; 
        let rName = namePool[Math.floor(Math.random() * namePool.length)];
        
        guildRecruit = { 
            isPlayer: false, name: rName, job: jName, rank: mRank, 
            level: player.level, hp: d.maxHp + (player.level*10), mp: d.maxMp + (player.level*5), 
            maxHp: d.maxHp + (player.level*10), maxMp: d.maxMp + (player.level*5), 
            atk: d.atk + (player.level*1), def: d.def + (player.level*1), 
            matk: d.matk + (player.level*1), str: d.str + (player.level*1), 
            dex: d.dex + (player.level*1), luk: d.luk + (player.level*1), int: d.int + (player.level*1),
            skills: d.skills 
        };
    }
    ...
    actionArea.innerHTML = `
        <div class="action-grid" style="grid-template-columns: 1fr;">
            <button class="btn" style="border-color:#4caf50;" onclick="acceptRecruit()">영입하기</button>
            <button class="btn" onclick="openGuild()">거절 (돌아가기)</button>
        </div>
    `;
}

function acceptRecruit() {
    player.party.push(guildRecruit);
    player.recruitUsedThisMonth = true; guildRecruit = null;
...
function showDismiss() {
    const sceneText = document.getElementById('scene-text');
    const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `[동료 방출]<br>방출 시 위로금 5,000G가 필요합니다.`;
    let html = `<div style="display:flex; flex-direction:column; gap:8px;">`;
    for(let i=1; i<player.party.length; i++) {
        html += `<button class="btn" style="border-color:#e53935;" onclick="dismissCompanion(${i})">${player.party[i].name} 방출 (5000G)</button>`;
    }
...
function dismissCompanion(index) {
    ...
    player.gold -= 5000; player.party.splice(index, 1);
    updateAllStats(); showMessage("위로금을 지급하고 동료를 방출했습니다.", showDismiss);
}
...
function openWorkplace() {
    ...
    const actionArea = document.getElementById('action-area');
    let currentDayOfMonth = ((player.day - 1) % 30) + 1;
    let daysToWork = 30 - currentDayOfMonth + 1;
    ...
    ["식당", "대장간", "약국"].forEach(w => {
        ...
        let btnTxt = prof >= 100 ? `${w} 운영 (준비중)` : `${w} 알바 (숙련도: ${prof})`;
        let onclick = prof >= 100 ? `showMessage('사업장 건축 기능은 준비 중입니다.')` : `doWork('${w}', ${daysToWork})`;
        html += `<button class="btn" onclick="${onclick}">${btnTxt}</button>`;
    });
    html += `</div><button class="btn" style="margin-top:12px; width:100%;" onclick="fadeTransition(renderTownUI)">돌아가기</button>`;
    actionArea.innerHTML = html;
}
...
function openShop() {
    const sceneText = document.getElementById('scene-text');
    const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `[상점]<br>소지금: ${player.gold}G<br>오늘의 원석 시세: ${player.orePrice}G/개<br><span style="color:#aaa; font-size:11px;">※ 장착 중인 아이템과 정수는 보이지 않습니다.</span>`;
    let html = `<div style="display:flex; flex-direction:column; gap:8px; max-height:200px; overflow-y:auto;">`;
    
    html += `<div class="inv-item"><div class="inv-header"><div class="inv-info">${icons.hp} HP포션 (250G)</div><button class="btn inv-btn" onclick="buyItem('hpPotion')">구매</button></div></div>`;
    html += `<div class="inv-item"><div class="inv-header"><div class="inv-info">${icons.mp} MP포션 (250G)</div><button class="btn inv-btn" onclick="buyItem('mpPotion')">구매</button></div></div>`;
...
function buyItem(type) {
    if(player.gold < 250) return showMessage("골드가 부족합니다.");
    player.gold -= 250; player.items[type]++; updateAllStats(); openShop();
}
...
function sellEssence(index, price) {
    player.gold += price; player.essenceList.splice(index, 1); updateAllStats(); openShop();
}

function sellEquip(index, price) {
    player.gold += price; player.equipList.splice(index, 1); updateAllStats(); openShop();
}
...
function tryEnterDungeon() {
    if (player.day > 1 && player.dungeonEnteredThisMonth) {
        ...
        let daysLeft = nextMonthDays - player.day;
        return showMessage(`이번 달 던전 입장은 이미 마쳤습니다.<br>다음 포탈이 열릴 때까지 ${daysLeft}일 남았습니다.`);
    }
    startDungeonExpedition();
}

function generateFloor() {
    player.pos = {x: 1, y: 1}; player.visited = ['1,1']; player.stepsLeft = 0; player.targetPos = null;
...
function startDungeonExpedition() {
    ...
    player.dungeonDay = 1; player.maxDungeonDay = 7; player.turn = 0;
    generateFloor(); updateDungeonTimer(); renderDungeonUI();
}
...
function getMinimapHTML() {
    ...
    for(let y=0; y<3; y++) {
        for(let x=0; x<3; x++) {
            ...
            else if(player.targetPos && player.targetPos.x === x && player.targetPos.y === y) classes += ' mm-target';
...
function renderDungeonUI() {
    ...
    if (player.stepsLeft > 0) {
        ...
        actionArea.innerHTML = `
            <div class="action-grid" style="grid-template-columns:1fr;">
                <div class="icon-btn" style="border-color:#888; padding:24px;" onclick="stepForward()"><span style="font-size:14px;">전진</span></div>
                <div class="icon-btn" onclick="openInventory('dungeon')">${icons.bag}<span>가방 확인</span></div>
            </div>`;
        return;
    }
    ...
    for(let y=0; y<3; y++) {
        for(let x=0; x<3; x++) {
            ...
            else if (isAdj) gridHtml += `<button class="compass-btn" onclick="startJourney(${x},${y},'${mapGrid[y][x].n}')"><span>${mapGrid[y][x].n}</span></button>`;
    ...
    if (player.monumentFound && player.pos.x === player.monumentPos.x && player.pos.y === player.monumentPos.y) {
        gridHtml += `
            <div class="action-grid" style="grid-template-columns:1fr; margin-top:12px;">
                <div class="icon-btn" style="border-color:#00e5ff; color:#00e5ff;" onclick="goNextFloor()">
                    ${icons.monument}<span>다음 층으로 올라가기</span>
                </div>
            </div>`;
    }
...
function showSleepMenu() {
    showMessage("어떻게 주무시겠습니까?", null, [
        {txt: "하룻밤 자기 (턴 초기화)", act: "closeModal(); sleepInDungeon(false)"},
        {txt: "던전 닫힐 때까지 푹 자기 (마을 복귀)", act: "closeModal(); sleepInDungeon(true)"},
        {txt: "취소", act: "closeModal()"}
    ]);
}

function sleepInDungeon(sleepUntilClose) {
    if (sleepUntilClose) {
        let daysLeft = player.maxDungeonDay + 1 - player.dungeonDay;
        passTime(daysLeft);
        player.inDungeon = false;
        let ts = getTotalStats();
        player.party[0].hp = ts.maxHp; player.party[0].mp = ts.maxMp;
        return fadeTransition(() => { showMessage("던전 포탈이 닫힐 때까지 푹 자고 무사히 마을로 귀환했습니다. (경험치 페널티 없음)", renderTownUI); });
    }
    ...
    player.turn = 0; player.hunger = 0; player.dungeonDay++;
    passTime(1);
    updateAllStats(); updateDungeonTimer();
    
    if (player.dungeonDay > player.maxDungeonDay) {
        player.inDungeon = false;
        let ts = getTotalStats(); player.party[0].hp = ts.maxHp; player.party[0].mp = ts.maxMp;
        fadeTransition(() => { showMessage("포탈 유지기간이 끝나 마을로 강제 귀환됩니다. (경험치 페널티 없음)", renderTownUI); });
    } else {
        renderDungeonUI();
    }
}

function startJourney(x, y, dirName) { player.targetPos = {x, y, name: dirName}; player.stepsLeft = 5; renderDungeonUI(); }

function goNextFloor() {
    player.floor++; player.maxDungeonDay += 5; generateFloor();
    fadeTransition(() => {
        showMessage(`${player.floor}층에 진입했습니다!<br>던전 유지 기간이 5일 추가되었습니다.`, () => { updateDungeonTimer(); renderDungeonUI(); });
    });
}

function stepForward() {
    player.turn++; player.stepsLeft--; updateAllStats();
    
    const checkArr = () => {
        if (player.stepsLeft <= 0) {
            player.pos = {x: player.targetPos.x, y: player.targetPos.y}; player.visited.push(`${player.pos.x},${player.pos.y}`);
            player.targetPos = null; 
            
            if (player.bossPos && player.pos.x === player.bossPos.x && player.pos.y === player.bossPos.y) {
    ...
    if (player.stepsLeft > 0) setTimeout(stepForward, 150); else checkArr();
}
...
function startCombat(isBoss = false) {
    ...
    if (isBoss) {
        let bTier = f + 3;
        let bName = floorBosses[f] || `심연의 군주`;
...
function renderCombatTurn(logMsg = "", isWaiting = false) {
    if (!isWaiting) {
        while(combatState.turnIndex < player.party.length && player.party[combatState.turnIndex].hp <= 0) {
            combatState.turnIndex++;
        }
    }
    
    if (combatState.turnIndex >= player.party.length && !isWaiting) {
        executeEnemyTurn(); return;
    }
    ...
    if (isWaiting) {
        actionArea.innerHTML = `<div style="text-align:center; padding:20px; color:#777;">( 잠시 대기... )</div>`;
        return;
    }

    let runBtn = "";
    if (combatState.turnIndex === 0) {
        runBtn = currentEnemy.isBoss ? `<div class="icon-btn" onclick="combatAction('run')">${icons.run}<span>도망</span></div>` : `<div class="icon-btn" onclick="showMessage('일반 몬스터와는 물러설 수 없습니다!')">${icons.run}<span style="color:#555">불가</span></div>`;
    }
...
function showSkillList() {
    ...
    let availableSkills = jobData[actor.job].skills.filter(s => actor.level >= s.lv);
    const sceneText = document.getElementById('scene-text');
    const actionArea = document.getElementById('action-area');
...
function combatAction(act, skillIdx) {
    ...
    if (act === 'attack') {
        dmg = Math.max(1, calcMemberDamage(actor, false) - currentEnemy.def); 
        log = `[${actor.name||actor.job}]의 공격! 적에게 ${dmg} 피해.`;
    } else if (act === 'skill') {
        let sk = jobData[actor.job].skills.filter(s => actor.level >= s.lv)[skillIdx];
        if (actor.mp < sk.mp) return renderCombatTurn("마나가 부족합니다!", false);
        actor.mp -= sk.mp; 
        dmg = Math.max(1, calcMemberDamage(actor, true, sk.mult) - currentEnemy.def); 
    ...
    playShakeEffect(() => {
        ...
        while(tempIdx < player.party.length) {
            if(player.party[tempIdx].hp > 0) { isAllDead = false; break; }
            tempIdx++;
        }
        if (isAllDead || combatState.turnIndex >= player.party.length) {
            setTimeout(executeEnemyTurn, 400); 
        } else {
            renderCombatTurn(log, false); 
        }
    });
}
...
function executeEnemyTurn() {
    ...
    if(alive.length === 0) return gameOver();
    ...
    playShakeEffect(() => {
        if (player.party[0].hp <= 0) return fadeTransition(() => { showMessage(`[치명타] 파티장의 체력이 0이 되었습니다...<br>가진 소모품과 골드를 잃고 마을로 실려갑니다.`, gameOver); });
...
function executeWin(lastLog) {
    ...
    if (currentEnemy.isBoss || Math.random() < dRate) {
        let newEq = generateEquip(dropTier); player.equipList.push(newEq);
        wLog += `<br><span style="color:#ffd54f">🎉 희귀한 장비 [${newEq.name}] 획득!</span>`;
    }

    if (currentEnemy.isBoss || Math.random() < dRate) {
        let mobName = currentEnemy.name.replace(/\[.*?층 군주\] |\[군주\] |\d+등급 /g, '');
        let ess = { id: Date.now()+Math.random(), name: `${dropTier}등급 ${mobName}의 정수`, tier: dropTier, passive: `[패시브] 올스탯 +${dropTier}% 및 ${mobName}의 기운` };
        player.essenceList.push(ess);
        wLog += `<br><span style="color:#00e5ff">💎 신비로운 [${ess.name}] 획득!</span>`;
    }
    
    if (currentEnemy.isBoss) { player.bossPos = null; }

    let goldGain = (currentEnemy.isBoss ? 300 : 50) * player.floor;
    let perGold = goldGain / player.party.length;
    player.gold += perGold;

    let oreGain = currentEnemy.isBoss ? 3 : 1;
    let perOre = oreGain / player.party.length;
    player.items.ore += perOre;
...
function gameOver() {
    let ts = getTotalStats();
    player.party[0].hp = ts.maxHp; player.party[0].mp = ts.maxMp; player.hunger = 0; player.fatigue = 0;
    player.items = { food: 0, hpPotion: 0, mpPotion: 0, ore: 0 }; 
...
function openInventory(context) {
    ...
    let ts = getTotalStats(); sceneText.innerHTML = `[가방 및 상태창] (현재 ${player.rank}등급 모험가)`;
    ...
    player.essenceList.forEach(ess => {
        let isEq = player.equipped["정수"].some(e => e.id === ess.id);
        html += `<div class="inv-item" style="${isEq ? 'border-color:#00e5ff;' : ''}"><div class="inv-header"><div class="inv-info">${icons.ore} <span style="color:${isEq ? '#00e5ff' : '#eee'};">[정수] ${ess.name} ${isEq ? '(장착중)' : ''}</span></div><button class="btn inv-btn" onclick="toggleEssence(${ess.id}, '${context}')">${isEq ? '파괴(100G)' : '장착'}</button></div><div class="inv-stats">${ess.passive}</div></div>`;
    });
...
function toggleEquip(id, context) {
    let eq = player.equipList.find(e => e.id === id);
    if (!eq) return;
    if (eq.type === '반지') {
        let idx = player.equipped['반지'].findIndex(r => r.id === id);
        if (idx > -1) player.equipped['반지'].splice(idx, 1);
        else {
            if (player.equipped['반지'].length >= 4) return showMessage("반지는 최대 4개까지만 착용 가능합니다.");
            player.equipped['반지'].push(eq);
        }
    } else {
        if (player.equipped[eq.type] && player.equipped[eq.type].id === id) player.equipped[eq.type] = null;
        else player.equipped[eq.type] = eq;
    }
...
function toggleEssence(id, context) {
    ...
    if (isEq) {
        showMessage("정말 파괴하시겠습니까? (100G 소모)", null, [
            {txt:"파괴하기", act:`executeDestroyEssence(${id}, '${context}')`}, {txt:"취소", act:"closeModal()"}
        ]);
    } else {
        if (player.equipped["정수"].length >= 2) return showMessage("정수는 최대 2개까지만 장착 가능합니다.");
        let ess = player.essenceList.find(e => e.id === id);
        player.equipped["정수"].push(ess); updateAllStats(); openInventory(context);
    }
}
...
function executeDestroyEssence(id, context) {
    ...
    player.gold -= 100;
    player.equipped["정수"] = player.equipped["정수"].filter(e => e.id !== id);
    player.essenceList = player.essenceList.filter(e => e.id !== id);
    updateAllStats(); openInventory(context);
}
...
function useItem(type, context) {
    ...
    if (type === 'food') {
        ...
        showMessage("식량 섭취! 허기짐이 100이 되었습니다.", () => openInventory(context));
    } else if (type === 'hpPotion') {
        player.items.hpPotion--; player.party[0].hp = Math.min(ts.maxHp, player.party[0].hp + 50);
        showMessage("HP 50 회복!", () => openInventory(context));
    } else if (type === 'mpPotion') {
        player.items.mpPotion--; player.party[0].mp = Math.min(ts.maxMp, player.party[0].mp + 30);
        showMessage("MP 30 회복!", () => openInventory(context));
    }
    updateAllStats();
}

function generateEquip(tier) {
    let type = equipTypes[Math.floor(Math.random() * equipTypes.length)];
    let numStats = Math.min(tier, 9);
    let statPool = ['atk', 'matk', 'str', 'dex', 'luk', 'int', 'maxHp', 'maxMp', 'def'];
    statPool.sort(() => 0.5 - Math.random());
    const minVals = [0, 1, 3, 8, 13, 20, 30, 45, 65, 90, 120];
    const maxVals = [0, 5, 10, 15, 25, 40, 60, 85, 115, 150, 200];
    let stats = {};
    for(let i=0; i<numStats; i++) stats[statPool[i]] = Math.floor(Math.random() * (maxVals[tier] - minVals[tier] + 1)) + minVals[tier];
