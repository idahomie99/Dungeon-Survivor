...
let player = {
    ...
    equipped: { "모자": null, "상의": null, "하의": null, "신발": null, "장갑": null, "무기": null, "망토": null, "팔찌": null, "목걸이": null, "반지": [], "정수": [], "펫": null }, // 💡 3. 펫 슬롯 추가
...
const icons = {
    ...
    pet: `<svg viewBox="0 0 24 24" fill="#4caf50"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>` // 임시 펫 아이콘
};
...
function ensureSaveCompatibility() {
    ...
    if(player.items.petBox === undefined) player.items.petBox = 0; // 💡 3. 펫 상자 추가
...
function passTime(days) {
    ...
    if (oldMonth !== player.month) {
        ...
        let tax = 1000 + Math.floor(totalWealth * 0.1); // 💡 2. 세금 10%
...
function showTownMenu() {
    ...
    let evClass = isNYEvent || true ? 'event-blink' : ''; // 💡 3. 상시 펫 뽑기 때문에 늘 반짝이게 둠 (또는 조건부)
    ...
    if(actionArea) {
        actionArea.innerHTML = `
            <div class="action-grid">
                ...
                <div class="icon-btn event-blink" onclick="openEventTab()">${icons.event}<span>이벤트</span></div>
            </div>
...
function openEventTab() {
    ...
    document.getElementById('scene-text').innerHTML = `<div class="scene-box">${getPlayerSprite(70)}<div>[이벤트 게시판]</div></div>`;
}
...
// 💡 4. 투기장 데미지 공식 완벽 동기화 (적도 나와 동일한 체급)
...
function openWorkplace() {
    ...
    sceneText.innerHTML = `<div class="scene-box">${getPlayerSprite(70)}<div>[일터]<br>현재 달의 남은 일수: ${daysToWork}일<br>근무 시 말일까지 연속으로 일합니다 (일당 50G).</div></div>`;
...
function openHospital() {
    setBackground('town');
    const sceneText = document.getElementById('scene-text'); const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `<div class="scene-box">${getPlayerSprite(70)}<div>[병원]<br>기절한 동료를 치료합니다.<br>비용: (동료의 레벨 x 200G)</div></div>`;
...
function openGuild() {
    setBackground('guild');
    const sceneText = document.getElementById('scene-text'); const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `<div class="scene-box">${getPlayerSprite(120)}<div>[용병 길드]<br>최대 4인 구성, 중복 직업 불가.</div></div>`;
    actionArea.innerHTML = `
...
function showRecruit() {
    ...
    const sceneText = document.getElementById('scene-text'); const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `<div class="scene-box"><img src="${jobData[guildRecruit.job].img}" style="width:70px; height:70px; margin-top:10px;"><br>[용병 영입]<br>${guildRecruit.rank}성 모험가 <b>${guildRecruit.name}</b> (Lv.${guildRecruit.level}, ${guildRecruit.job}) 가 파티 합류를 원합니다.<br><span style="color:#ffc107;">영입 비용: ${f(cost)}G</span></div>`;
...
function showDismiss() {
    const sceneText = document.getElementById('scene-text'); const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `<div class="scene-box">${getPlayerSprite(70)}<div>[동료 방출]<br>위로금 5,000G가 필요합니다.</div></div>`;
    let html = `<div style="display:flex; flex-direction:column; gap:8px;">`;
...
function openShop() {
    setBackground('town');
    const sceneText = document.getElementById('scene-text'); const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `<div class="scene-box">${getPlayerSprite(70)}<div>[상점]<br>소지금: ${f(player.gold)}G<br>오늘의 원석 시세: ${f(player.orePrice)}G/개</div></div>`;
...
function openWarehouse() {
    setBackground('town');
    const sceneText = document.getElementById('scene-text'); const actionArea = document.getElementById('action-area');
    sceneText.innerHTML = `<div class="scene-box">${getPlayerSprite(70)}<div>[마을 창고]<br>골드와 아이템을 안전하게 보관합니다.</div></div>`;
...
function executeHolyArrow(targetIdx, skillIdx) {
    ...
    if (currentEnemy.isArena) rawDmg = Math.floor(rawDmg * 0.2); // 💡 4. 투기장 80% 뎀감
...
function openInventory(context) {
    ...
    // 💡 3. 펫 상자 개봉 
    if(player.items.petBox > 0) {
        html += `<div class="inv-item"><div class="inv-header"><div class="inv-info">🐾 펫 소환 상자 (${f(player.items.petBox)})</div><button class="btn inv-btn" style="border-color:#4caf50; color:#4caf50;" onclick="usePetBox('${context}')">열기</button></div><div class="inv-stats">다양한 등급과 타입의 펫이 나옵니다.</div></div>`;
    }
    ...
    player.equipList.forEach((eq, idx) => { 
        let statText = eq.type === '펫' ? `[${eq.petEffect.type}] 성능: ${eq.petEffect.value * 100}%` : Object.keys(eq.stats).map(k => `${statNames[k]}+${f(eq.stats[k])}`).join(', '); 
...
function usePetBox(context) {
    ...
    let types = ['회복형', '방어형', '공격형'];
    let type = types[Math.floor(Math.random() * types.length)];

    let pet = {
        id: Date.now() + Math.random(),
        type: '펫',
        name: `<span style="color:${color}">[${tierName}] ${type} 펫</span>`,
        tier: tier,
        stats: {}, 
        petEffect: { type: type, value: val, tierName: tierName }
    };
    player.equipList.push(pet);
    updateAllStats();
